...
class Linescore_extractor extends tableExtractor {
	var $boxscore	= '';
	var $win		= '';
	var $loss		= '';
	var $save		= '';

	function Linescore_extractor($linescore) {
		$this->source	= $linescore;
		$this->extractTable();

		// extract url
		$startURL	= stripos($this->source, '<a href="') + 9;
		$endURL		= stripos($this->source, '">Boxscore</a>');
		$this->boxscore = substr($this->source, $startURL, $endURL - $startURL);

		// Extract win.
		$start	= stripos($this->source, '<strong>W:</strong> ') + 20;
		$end	= stripos($this->source, '<win />');
		$this->win = substr($this->source, $start, $end - $start);

		// Extract loss.
		$start	= stripos($this->source, '<strong>L:</strong> ') + 20;
		$end	= stripos($this->source, '<loss />');
		$this->loss = substr($this->source, $start, $end - $start);

		// Extract save.
		$start	= stripos($this->source, '<strong>S:</strong> ') + 20;
		$end	= stripos($this->source, '<save />');
		if (!empty($start) && !empty($end)) {
			$this->save = substr($this->source, $start, $end - $start);
		}
	}
	...
	function extractTable() {
		...
		return $this->prepare_linescore();
	}

	function prepare_linescore() {
		$this->finalArray['boxscore']	= $this->boxscore;
		$this->finalArray['win']		= $this->win;
		$this->finalArray['loss']		= $this->loss;
		$this->finalArray['save']		= $this->save;

		$this->finalArray['1']['R'];
		// Strips <strong> and </strong>
		$search = array(
			'<strong>',
			'</strong>'
		);
		$this->finalArray['1']['R'] = str_replace($search, "", $this->finalArray['1']['R']);
		$this->finalArray['1']['H'] = str_replace($search, "", $this->finalArray['1']['H']);
		$this->finalArray['1']['E'] = str_replace($search, "", $this->finalArray['1']['E']);
		$this->finalArray['2']['R'] = str_replace($search, "", $this->finalArray['2']['R']);
		$this->finalArray['2']['H'] = str_replace($search, "", $this->finalArray['2']['H']);
		$this->finalArray['2']['E'] = str_replace($search, "", $this->finalArray['2']['E']);
